  ...
  try {
    ...
    if (filterPath) {
      ...
      if (matchingDirs.size === 0) {
        console.error(`Error: No directories named "${filterName}" found in ${PHP_FOLDER}`);
        process.exit(1);
      }
    ...
    const doneLines = fileStats.filter((f) => f.isDone).reduce((sum, f) => sum + f.lines, 0);
    ...
    const functionsPercent = totalFunctions > 0 ? (doneFunctions / totalFunctions) * 100 : 0;
    ...
    console.log(`    ⊞ Files: ${doneFiles}/${totalFiles} (${filesPercent.toFixed(2)}%)`);
    console.log(
      `    ƒ Functions: ${doneFunctions}/${totalFunctions} (${functionsPercent.toFixed(2)}%)`,
    );
    console.log(`    ≡ Lines: ${doneLines}/${totalLines} (${linesPercent.toFixed(2)}%)`);
