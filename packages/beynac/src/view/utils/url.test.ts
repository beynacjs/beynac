...
describe("url", () => {
	...
	describe("getPath", () => {
		...
		it("getPath - http+unix", () => {
			const path = getPath(
				new Request("http+unix://%2Ftmp%2Fsocket%2Esock/hello/"),
			);
			expect(path).toBe("/hello/");
		});
	});
	...
	describe("getQueryParam", () => {
		it("Parse URL query strings", () => {
			...
			expect(
				getQueryParam("http://example.com/?name=hey#fragment", "name"),
			).toBe("hey#fragment");
			expect(
			...
			expect(getQueryParam("http://example.com/?name=%E0%A4%A", "name")).toBe(
				"%E0%A4%A",
			);

			expect(
				getQueryParam("http://example.com/?name0=sam&name1=tom", "name0"),
			).toBe("sam");
			expect(
				getQueryParam("http://example.com/?name0=sam&name1=tom", "name1"),
			).toBe("tom");
			expect(
				getQueryParam("http://example.com/?name0=sam&name1=tom", "name"),
			).toBe(undefined);
			...
			expect(
				getQueryParam("http://example.com/?name=hey&age=20&tall=170"),
			).toEqual({
				name: "hey",
				age: "20",
				tall: "170",
			...
			expect(
				getQueryParam("http://example.com/?name=sam&name=tom", "name"),
			).toBe("sam");
	...
	describe("getQueryParams", () => {
		it("Parse URL query strings", () => {
			...
			expect(
				getQueryParams("http://example.com/?name=hey#fragment", "name"),
			).toEqual(["hey#fragment"]);
			expect(
				getQueryParams("http://example.com/?name=hey&name=foo", "name"),
			).toEqual(["hey", "foo"]);
			expect(
			...
			).toEqual(["a web framework"]);
			expect(
				getQueryParams("http://example.com/?name=%E0%A4%A", "name"),
			).toEqual(["%E0%A4%A"]);
