...
let nameSpaceContext: JSXContext<string> | undefined;
...
const applyNodeObject = (
	...
	const childNodes = (
		isNew ? undefined : container.childNodes
...
export const build = (
	...
	try {
		...
		if (
			...
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			foundErrorHandler = (children[0] as any)[
				DOM_ERROR_HANDLER
			] as ErrorHandler;
				...
				if (oldVChildren && oldVChildren.length) {
					const i = oldVChildren.findIndex(
						isNodeString(child)
							...
							: child.key !== undefined
								? (c) =>
										c.key === (child as Node).key &&
										c.tag === (child as Node).tag
...
const updateMap: WeakMap<NodeObject, [UpdateMapResolve, Function]> =
	new WeakMap<NodeObject, [UpdateMapResolve, Function]>();
