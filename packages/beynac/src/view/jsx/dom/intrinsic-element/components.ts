...
let blockingPromiseMap: Record<string, Promise<Event> | undefined> =
	Object.create(null);
...
const documentMetadataTag = (
	...
	let existingElements: NodeListOf<HTMLElement> | undefined;
	...
	const ref = composeRef(props.ref, (e: HTMLElement) => {
		...
		let promise = (blockingPromiseMap[e.getAttribute(key) as string] ||=
	...
	if (supportBlocking && blocking === "render") {
		...
		if (props[key]) {
			...
			const promise = (blockingPromiseMap[value] ||= new Promise<Event>(
				(resolve, reject) => {
