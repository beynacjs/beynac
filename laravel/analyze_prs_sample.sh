...
# Script to analyze PR sizes in the Laravel repository (sample version)
# This script extracts a sample of merged PRs and calculates statistics on their sizes

echo "Analyzing PR sizes in the Laravel repository (sample)..."

# Get a sample of merge commits (last 500)
echo "Extracting merge commits sample..."
git log --merges --format="%H" -n 500 > merge_commits_sample.txt
...
count=0
total_commits=$(wc -l < merge_commits_sample.txt)
...
rm merge_commits_sample.txt pr_sizes.txt pr_sizes_sorted.txt histogram.csv
